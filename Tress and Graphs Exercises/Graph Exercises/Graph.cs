...
namespace Graph_Exercises
{
    class GraphTest
    {
        static void Main(string[] args)
        {
            ...
            List<int> path = graph.FindShortestPath(3, 5);
            bool isCyclic = graph.IsCyclic();
            graph.UndirectedDFSTraversal();
            Console.ReadKey();
        }
    }
    ...
    class GraphComponents
    {
        ...
        public void UndirectedDFSTraversal()
        {
        ...
        public void Traverse(int v)
        {
            if (!visited[v])
            {
                Console.Write(v + " ");
                visited[v] = true;
                foreach (int path in graph.childNodes[v])
                {
                    Traverse(path);
                }
            }
